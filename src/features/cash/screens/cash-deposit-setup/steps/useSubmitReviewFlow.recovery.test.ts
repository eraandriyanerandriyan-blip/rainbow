...
beforeEach(() => {
  ...
  sessionStore().setIdentity(IDENTITY);
  sessionStore().setGovernmentId(GOVERNMENT_ID);
...
describe('useSubmitReviewFlowStore.submit recovery', () => {
  ...
  it('keeps the recovery inputs available after an identity mismatch', async () => {
    ...
    expect(sessionStore().session).toMatchObject({ status: 'recovery', identity: IDENTITY, governmentId: GOVERNMENT_ID });
  });
  ...
  it('returns to OTP entry without replacing the session when the code is invalid', async () => {
    ...
    expect(track).toHaveBeenCalledWith('cash.phone_verify_failed', { mode: 'recovery', reason: 'invalidCode' });
  });
